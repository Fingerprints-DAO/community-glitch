...
contract TestHelpers is PRBTest {
  ...
  uint256 internal constant MAX_TOP_BIDS = 50;
  ...
  function fillTopBids(GlitchAuction auction) internal {
    ...
    for (uint256 i = 0; i < MAX_TOP_BIDS; i++) {
      uint256 loopIndex = i % 10;
      address bidder = addresses[loopIndex];
      uint256 bidAmount = bidAmounts[loopIndex] + (i * 0.1 ether);
